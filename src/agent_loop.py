...
- Atlas / worlds / entities / datasets / catalogue / structured data → use `manage_atlas` (NEVER app_api for /api/atlas/* — blocked). describe_world, get_schema, find, list_queries, create_query, update_query are **actions inside manage_atlas**, not separate tools. world_id accepts UUID, prefix, or exact world name (e.g. "CG-BMS"); entity_name for collections. Orient with describe_world/find_world — NOT list_worlds unless user wants all worlds. get_schema before joins (returns field slugs + sample doc). SQL: list_queries → get_schema on sources → create_query/update_query with MySQL SELECT. Read with find/findOne/countDocuments/execute_query; Write with insertOne/updateOne. Do NOT store tabular data in `manage_memory`. Return `[Name](#atlas-entity-<id>)` or `[World](#atlas-world-<id>)` links after create.
...
- When you reference an entity by ID in your reply, render it as a STANDARD markdown link with a hash-prefixed anchor. The frontend converts these into clickable jump buttons:
  ...
  - Atlas worlds: `[World name](#atlas-world-<id>)`
  - Atlas entities: `[Entity name](#atlas-entity-<id>)`
...
- "Open/show <panel>" (documents, library, gallery, email, inbox, sessions, brain/memories, skills, settings, notes, cookbook, atlas) → call `ui_control` with `open_panel <name>`. Panel aliases: library/doc/docs/document→documents, images→gallery, mail/inbox/emails→email, chats/history→sessions, memory/memories→brain, preferences→settings, models/serve/serving→cookbook, atlas/data/catalogue→atlas. CRITICAL: "open memory/memories/brain" / "open skills" / "open notes" / "open documents" / "open cookbook" / "open atlas" means OPEN THE PANEL — call `ui_control`, NOT a manage/list tool. The "manage_*" tools list contents in chat; `ui_control open_panel` opens the visual modal the user is asking for.
...
```
Atlas store — ONE tool; describe_world, get_schema, find, list_queries, create_query are JSON actions here (not separate tools). app_api /api/atlas/* is blocked. get_schema before joins; SQL uses collection names as tables. Example join update:
```manage_atlas
{"action": "update_query", "world_id": "CG-BMS", "query_name": "Query 1", "sql_text": "SELECT a.*, b.deal_id FROM SIT_Siebel_BRM_Products a JOIN SIT_Deal_Products b ON a.part_number = b.part_number"}
```""",
...
async def stream_agent_loop(
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        for i, block in enumerate(tool_blocks):
            ...
            _eid = result.get("entity_id")
            if _eid and block.tool_type == "manage_atlas":
                _ename = (result.get("entity_name") or "").strip()
                _label = _ename or "View entity"
                _anchor = f"\n\n[{_label}](#atlas-entity-{_eid})\n"
                yield 'data: ' + json.dumps({"delta": _anchor}) + '\n\n'

            _wid = result.get("world_id")
            if _wid and block.tool_type == "manage_atlas" and result.get("entity_id") is None:
                _wname = (result.get("world_name") or "").strip()
                _label = _wname or "View world"
                _anchor = f"\n\n[{_label}](#atlas-world-{_wid})\n"
                yield 'data: ' + json.dumps({"delta": _anchor}) + '\n\n'
