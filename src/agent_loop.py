...
- Atlas / worlds / entities / datasets / catalogue / structured data → use `manage_atlas` (NEVER app_api for /api/atlas/*). world_id accepts UUID, prefix, or exact world name (e.g. "CG-BMS"); entity_name for collections. Orient with describe_world/find_world — NOT list_worlds unless user wants all worlds. list_entities returns summary by default; get_schema for field details. Read with find/findOne/countDocuments; Write with insertOne/updateOne; deleteMany needs confirm if large. Do NOT store tabular data in `manage_memory`. Return `[Name](#atlas-entity-<id>)` or `[World](#atlas-world-<id>)` links after create.
...
```
Atlas document store. world_id: UUID, prefix, or exact world name. Orient: describe_world, find_world. Read: find/findOne/countDocuments/search — get_schema only if fields unknown. Write: insertOne/updateOne; deleteMany needs confirm. format=compact|table saves tokens.""",
