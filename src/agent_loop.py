...
- Atlas / worlds / entities / datasets / catalogue / structured data → use `manage_atlas` (MongoDB-style: get_schema, find, updateOne, insertOne, filter by `_id`). Do NOT store tabular data in `manage_memory`. Return `[Name](#atlas-entity-<id>)` or `[World](#atlas-world-<id>)` links after create.
...
- Atlas / worlds / entities / datasets / catalogue / structured data → use `manage_atlas` (MongoDB-style: get_schema, find, updateOne, insertOne, filter by `_id`). Do NOT store tabular data in `manage_memory`. Return `[Name](#atlas-entity-<id>)` links after create.
...
```
MongoDB-style Atlas: get_schema first, then find/updateOne/insertOne with filter objects. Each document has auto _id. Actions: find, findOne, insertOne, insertMany, updateOne, updateMany, deleteOne, deleteMany, countDocuments, get_schema, create_entity, import_rows, export_csv. world_id optional. Return [Name](#atlas-entity-<id>) links after create.""",
