...
- Atlas / worlds / entities / datasets / catalogue / structured data → use `manage_atlas`. Do NOT store tabular data in `manage_memory` or Documents unless the user wants a document. Use `import_rows` for bulk CSV. Default `limit=20` on `list_rows`. Return `[Name](#atlas-entity-<id>)` or `[World](#atlas-world-<id>)` links after create.
...
- Atlas / worlds / entities / datasets / catalogue / structured data → use `manage_atlas`. Do NOT store tabular data in `manage_memory` or Documents unless the user wants a document. Use `import_rows` for bulk CSV. Default `limit=20` on `list_rows`. Return `[Name](#atlas-entity-<id>)` links after create.
...
Notes, checklists, AND user reminders. Use this for "create/add/write a note", todos, checklists, and "remind me to X at <time>" — never use memory for note content. For reminders, pair a short `title` (what to do) with a `due_date` (when). `due_date` accepts natural language ("tomorrow at 1pm", "in 2 hours", "next monday 9am") or ISO ("2026-05-12T13:00:00"). Actions: `list`, `add` (title, content OR items:[{text,done}], note_type, color, label, due_date), `update`, `delete`, `toggle_item`.""",
    "manage_atlas": """\
```manage_atlas
{"action": "create_entity", "name": "Customers", "attributes": [{"name": "id", "type": "text", "primary_key": true}, {"name": "email", "type": "text"}]}
```
Atlas structured data: worlds, entities (schemas), rows. Use for datasets/catalogues — NOT manage_memory. Actions: list_worlds, create_world, list_entities, create_entity, add_attribute, list_rows (limit 20 default), count_rows, add_row, update_row, delete_row, import_rows (csv + mode), export_csv. world_id optional (default world). Return [Name](#atlas-entity-<id>) links after create.""",
