...
FUNCTION_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            "name": "manage_atlas",
            "description": "Atlas document/SQL store. ONE tool — describe_world, get_schema, find, list_queries, create_query, update_query, execute_query are actions here (not separate tools). NEVER use app_api for /api/atlas/* (blocked). world_id: UUID, prefix, or exact name. get_schema for field slugs + sample doc before joins. SQL queries use MySQL-flavored SELECT; collection names are table names.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {
                        "type": "string",
                        "enum": [
                            "list_worlds", "find_world", "create_world", "describe_world",
                            "list_entities", "find_entity", "create_entity",
                            "get_schema", "describe_collection",
                            "find", "findOne", "countDocuments", "search",
                            "insertOne", "insertMany",
                            "updateOne", "updateMany", "replaceOne",
                            "deleteOne", "deleteMany",
                            "create_relationship", "list_relationships",
                            "list_keys", "create_key",
                            "list_queries", "create_query", "update_query", "execute_query",
                            "list_clusters", "create_cluster", "assign_entity_to_cluster",
                            "import_rows", "export_csv",
                            "list_rows", "count_rows", "add_row", "update_row", "delete_row",
                        ],
                        "description": "describe_world/list_entities for discovery; get_schema for field names; find/findOne/countDocuments for reads; list_queries/create_query/update_query/execute_query for SQL; insertOne/updateOne for writes",
                    },
                    "world_id": {"type": "string", "description": "World UUID, 8-char prefix, or exact world name (optional; default world if omitted)"},
                    "world_name": {"type": "string", "description": "Exact world name lookup (alternative to world_id)"},
                    "entity_id": {"type": "string", "description": "Entity/collection UUID or 8-char prefix"},
                    "entity_name": {"type": "string", "description": "Entity/collection name (exact match)"},
                    "name": {"type": "string", "description": "World or entity name (create actions, or find_world/find_entity query)"},
                    "query": {"type": "string", "description": "Search query for find_world, find_entity, or search"},
                    "description": {"type": "string", "description": "World or entity description"},
                    "filter": {"type": "object", "description": "MongoDB filter e.g. {\"status\":\"active\"}, {\"name\":{\"$contains\":\"smith\"}}, {\"state\":{\"$in\":[\"NY\",\"CA\"]}}"},
                    "document": {"type": "object", "description": "Document for insertOne"},
                    "documents": {"type": "array", "items": {"type": "object"}, "description": "Documents for insertMany"},
                    "update": {"type": "object", "description": "Update patch or {$set:{},$unset:[],$inc:{}}"},
                    "replacement": {"type": "object", "description": "Full replacement document for replaceOne"},
                    "row": {"type": "object", "description": "Legacy alias for document"},
                    "row_id": {"type": "integer", "description": "Legacy _id shortcut for updateOne/deleteOne"},
                    "_id": {"type": "integer", "description": "Document auto id"},
                    "limit": {"type": "integer", "description": "Max documents or search hits (default 20, max 100 for find)"},
                    "offset": {"type": "integer", "description": "Pagination offset"},
                    "format": {"type": "string", "enum": ["json", "compact", "table"], "description": "find/findOne output format (default json)"},
                    "fields": {"type": "array", "items": {"type": "string"}, "description": "Project only these document fields in find/findOne"},
                    "include_fields": {"type": "boolean", "description": "list_entities/describe_world: include field slug list (default false)"},
                    "include_stats": {"type": "boolean", "description": "get_schema: include occurrence_count and nullable_ratio per field"},
                    "include_sparse": {"type": "boolean", "description": "get_schema: include ultra-sparse fields (nullable_ratio>0.9, occurrence_count<=1)"},
                    "sort": {"type": "object", "description": "Sort spec e.g. {\"_id\": -1}"},
                    "upsert": {"type": "boolean", "description": "Create if no match (updateOne/replaceOne)"},
                    "confirm": {"type": "boolean", "description": "Required for deleteMany on large/unfiltered sets"},
                    "from_entity_id": {"type": "string"},
                    "to_entity_id": {"type": "string"},
                    "from_entity_name": {"type": "string"},
                    "to_entity_name": {"type": "string"},
                    "rel_type": {"type": "string", "enum": ["one_to_one", "one_to_many", "many_to_one", "many_to_many"]},
                    "from_cardinality": {"type": "string", "enum": ["one", "many", "one_or_zero"]},
                    "to_cardinality": {"type": "string", "enum": ["one", "many", "one_or_zero"]},
                    "from_field": {"type": "string"},
                    "to_field": {"type": "string"},
                    "cluster_id": {"type": "string", "description": "Cluster UUID or prefix"},
                    "cluster_name": {"type": "string", "description": "Exact cluster name"},
                    "parent_cluster_id": {"type": "string"},
                    "parent_cluster_name": {"type": "string"},
                    "color": {"type": "string", "description": "Cluster accent: blue, green, amber, rose, violet"},
                    "csv": {"type": "string", "description": "CSV text for import_rows"},
                    "mode": {"type": "string", "enum": ["append", "merge", "replace"], "description": "Import mode"},
                    "sql_text": {"type": "string", "description": "MySQL-flavored SELECT for create_query/update_query"},
                    "query_id": {"type": "string", "description": "Query UUID or prefix"},
                    "query_name": {"type": "string", "description": "Exact saved query name (lookup for execute_query/update_query)"},
                    "rename": {"type": "string", "description": "New display name for update_query"},
                },
                "required": ["action"],
            },
        },
