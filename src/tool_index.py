...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "manage_atlas": "Atlas structured data store (worlds, collections, SQL queries, documents). ONE tool — describe_world, get_schema, find, list_queries, create_query, update_query, execute_query are JSON actions inside manage_atlas, NOT separate tools. NEVER use app_api for /api/atlas/* (blocked). world_id: UUID, prefix, or exact name (e.g. CG-BMS). entity_name for collections. Orient with describe_world; field names with get_schema (sample doc + slugs); reads with find/countDocuments; SQL joins with create_query/update_query (MySQL-flavored SELECT). Preview limits rows but total is in execute_query response.",
    ...
    "app_api": "Generic loopback to allowed Odysseus internal endpoints. Use this when the user wants something the UI can do but there's no named tool for it. Covers calendar, gallery, library/documents, memory, notes, tasks, settings, research, compare, cookbook GPUs/state — allowed UI buttons hit /api/* endpoints and you can hit them too. Sensitive auth/user/admin/shell paths and host-control Cookbook mutation routes are blocked; do NOT use app_api for shell commands, package installs, engine rebuilds, or PID signalling. Use named command tooling for shell commands. action='endpoints' with filter=<keyword> lists available endpoints. action='call' takes method+path+body. Hits same routes the UI uses — auth flows free. ATLAS: /api/atlas/* is BLOCKED — use manage_atlas instead (describe_world, get_schema, find, list_queries, create_query, etc. are actions on that one tool). NOTE: themes are NOT an API endpoint — use the ui_control tool (create_theme / set_theme), not app_api. SESSIONS/CHATS: do NOT use app_api for these — GET /api/sessions returns EMPTY for tool calls (it's owner-filtered and tool calls authenticate as a different identity). EMAIL ACCOUNTS: do NOT use /api/email/accounts via app_api; use list_email_accounts, list_emails, and read_email instead. To list/rename/archive/delete/fork chats use the list_sessions and manage_session tools instead.",
...
class ToolIndex:
    ...
    _KEYWORD_HINTS = {
        ...
        # Atlas / structured data — worlds, collections, queries, schemas, joins.
        # Without this, RAG often surfaces app_api (always-on) but NOT manage_atlas,
        # and /api/atlas/* is blocked — leaving the agent stuck.
        frozenset({"atlas", "world", "worlds", "collection", "collections",
                   "entity", "entities", "catalogue", "catalog", "dataset",
                   "datasets", "compass", "get_schema", "describe_world",
                   "list_entities", "manage_atlas", "row count", "documents in",
                   "sql query", "join", "siebel", "brm", "sit_", "bms"}):
            {"manage_atlas"},
        frozenset({"modify the query", "update the query", "change the query",
                   "create a query", "list queries", "execute query",
                   "saved query", "atlas query"}):
            {"manage_atlas"},
    }
